...
public class JOEmojiableBtn: UIButton {
    ...
    var EmoticonTap:UITapGestureRecognizer!
    ...
    private func activate(){
        if !active {
            if dataset != nil {
                ...
                EmoticonTap = UITapGestureRecognizer(target: self, action: "EmoticonSelect:")
                bgClear.addGestureRecognizer(EmoticonTap)
                bgClear.userInteractionEnabled = true
                ...
                let midscreen = (self.superview?.bounds.maxX)!/2
                
                
                if origin.x < midscreen {
                    options = UIView(frame: CGRectMake(origin.x, origin.y - (sizeBtn.height), sizeBtn.width, sizeBtn.height))
                }
                    
                else if origin.x > midscreen {
                    options = UIView(frame: CGRectMake(origin.x - (sizeBtn.width), origin.y - (sizeBtn.height), sizeBtn.width, sizeBtn.height))
                }
                ...
                //self.superview?.addSubview(options)
    ...
    func selectIndex(index:Int){
        ...
        SelectionEmoticonImageData = UIImage(named: dataset[index].image)
    }
...
extension JOEmojiableBtn:SelectorViewDelegate, UIGestureRecognizerDelegate {
    
    
    public func movedTo(point:CGPoint){
        let t = options.frame.width/CGFloat(dataset.count)
        if (point.y < (options.frame.minY - 50) || point.y > options.frame.maxY ){
            loseFocus()
        }else{
            if (point.x - options.frame.minX > 0 && point.x < options.frame.maxX){
                selectIndex(Int(floor((point.x - options.frame.minX)/t)))
            }else{
                loseFocus()
            }
        }
    ...
    func EmoticonSelect(gestureReconizer: UITapGestureRecognizer) {
        
        print("test")
        var point = gestureReconizer.locationInView(self.bgClear)
        /*
         let t = options.frame.width/CGFloat(dataset.count)
         print(point.y)
         print(options.frame.minY)
    ...
    /**
     ...
     public func movedTo(point:CGPoint){
     let t = options.frame.width/CGFloat(dataset.count)
     print(Int(round((point.x-origin.x)/t)))
